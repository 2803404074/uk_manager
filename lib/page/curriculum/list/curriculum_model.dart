...
class CurriculumModel extends BaseModel{
  ...
  void loadList(bool loadMore,{int? currentIndex,String? queryKey,int? status,int? hasVip}){
    ...
    HttpProxy.httpProxy.post(Api.getCurriculumList, parameters: parameter.toJson()).then((value){
      if(value.code == 200){
  ...
  void getTypeList(){
    HttpProxy.httpProxy.post(Api.getCurriculumTypeList,parameters: {}).then((res){
      if(res.code == 200){
        res.data.forEach((element) {
          fourList.add(element);
        });
        fourList.insert(0, {'name':'全部',});
        fourCurrentIndex = 0;
        loadList(false);
      }
